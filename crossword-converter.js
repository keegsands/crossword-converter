    ...
    if(myArgs.length > 0){ // If there is an argument then use it
        convertCrossword(myArgs[0]);
    }else{ // If not arg then use the default file name
        convertCrossword('./sample-data/default-crossword.json');
    }
    ...
    text puzzle file */
    function convertCrossword(filename){
        ...
        fs.writeFileSync(filename.replace('.json', '.txt'), output);
    }
